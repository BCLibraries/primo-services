...
class PNXTranslatorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testLocalRecordLoadsCorrectly()
    {
        ...
        $this->assertEquals(["Otters–Juvenile literature"], $result_0->subjects);
        $this->assertEquals(['Juvenile literature'], $result_0->genres);

        $result_1 = $result[1];
        $this->assertEquals(['Guy Troughton'], $result_1->contributors);
        $this->assertEquals(
            [
        ...
        $this->assertEquals(['eng'], $result_1->languages);
        ...
        $this->assertEquals(["9781429600347", "1429600349"], $result_4->isbn);
        $this->assertEquals(['77485984'], $result_4->oclcid);
